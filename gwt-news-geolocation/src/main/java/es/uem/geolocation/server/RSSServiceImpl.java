...
public class RSSServiceImpl extends RemoteServiceServlet implements RSSService {
	...
	private final String NEWS_TYPE_NATIONAL = "National"; 
	private final String NEWS_TYPE_INTERNATIONAL = "International";
	 ...
	 */
	public RSS loadLocalRSSNews(String type) {
		RSS rssSource = null; 
		...
		try {
			...
			if (type.equals(NEWS_TYPE_NATIONAL)) {
				// Nacional
				...
				items.add(new Article("Aamería - INMIGRACIÓN ILEGAL","La Guardia Civil ha detenido, a primeras horas de hoy, a 21 inmigrantes tras desembarcar desde dos embarcaciones tipo zodiac en Adra  y en la zona del Cabo de Gata. ","#",new java.util.Date(1239127837248L)));
				...
				items.add(new Article("Páis Vasco - VIOLENCIA CALLEJERA.","  La sede del PSE - EE en Zumaya (Guipúzcoa), los juzgados de Portugalete (Vizcaya) y un local de Telefónica en Plencia (Vizcaya) han sido atacados esta madrugada con artefactos caseros y cócteles molotov, provocando escasos daños materiales y ninguno personal.","#",new java.util.Date(1294351835484L)));
				items.add(new Article("Granada - INMIGRACIÓN ILEGAL","El Servicio de Vigilancia Aduanera ha interceptado una patera con 55  inmigrantes de origen magrebí en la costa de Salobreña.   ","#",new java.util.Date(1204297828500L)));
			...
			else if (type.equals(NEWS_TYPE_INTERNATIONAL)) { 			
				// Internacional
				...
				items.add(new Article("EE.UU. - LIBERTAD DURADERA","Pequeñas cantidades de esporas de carbunco se han encontrado en la principal oficina postal de West Palm Bean (Florida).","#",new java.util.Date(1286680208484L)));
	...
	public RSS loadRSSNews(String uri) {
		RSS rssSource = null;		
		try { 
			if (uri.equals("localNacional")) {
				// Local
				rssSource = loadLocalRSSNews(NEWS_TYPE_NATIONAL);
			}
			else if (uri.equals("localInternacional")) {
				// Local 
				rssSource = loadLocalRSSNews(NEWS_TYPE_INTERNATIONAL); 						
			} else {
