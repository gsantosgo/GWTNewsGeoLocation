...
public class MyWebService {
	private static Logger logger = Logger.getLogger("es.uem.geonames.ws");
	...
	private static Toponym getToponymContinentFromElement(Element toponymElement) {
		ToponymContinent toponym = new ToponymContinent();
		toponym.setName(toponymElement.getChildText("name"));
		toponym.setAlternateNames(toponymElement.getChildText("alternateNames"));
		toponym.setLatitude(Double.parseDouble(toponymElement
				.getChildText("lat")));
	 ...
	 * @return
	 */
	private static Toponym getToponymCountryFromElement(Element toponymElement) {
	...
	public static ToponymSearchResult search(
		...
		searchResult.setToponyms(toponyms);
	 ...
	 * @throws Exception
	 */
	public static ToponymSearchResult searchContinent(
			ToponymSearchCriteria searchCriteria) throws Exception {
		ToponymSearchResult searchResult = new ToponymSearchResult();
		...
		for (Object obj : root.getChildren("geoname")) {						
			Element toponymElement = (Element) obj;
			Toponym toponym = getToponymContinentFromElement(toponymElement);
			toponym.setStyle(searchResult.getStyle());
			toponyms.add(toponym); 			 
		}
		searchResult.setToponyms(toponyms);
	...
	public static ToponymSearchResult countryInfo(ToponymSearchCriteria searchCriteria) throws Exception {
		ToponymSearchResult searchResult = new ToponymSearchResult();
		...
		for (Object obj : root.getChildren("country")) {						
			Element toponymElement = (Element) obj;
			Toponym toponym = getToponymCountryFromElement(toponymElement);
			toponym.setStyle(searchResult.getStyle());
			toponyms.add(toponym); 			 
		}
		searchResult.setTotalResultsCount(toponyms.size()); 		
		searchResult.setToponyms(toponyms);
